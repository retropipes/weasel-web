...
import com.puttysoftware.weaselweb.pluginmanagers.PluginRegistration;
import com.puttysoftware.weaselweb.prefs.PreferencesManager;

public class MenuManager {
    // Fields
    private JMenuBar mainMenuBar;
    private JMenu fileMenu, editMenu, playMenu, gameMenu, editorMenu, pluginMenu, helpMenu;
    private JMenu editorFillSubMenu;
    private JMenuItem fileNew, fileOpen, fileOpenLocked, fileClose, fileSave, fileSaveAs, fileSaveLocked, fileExit;
    ...
    private JMenuItem gameObjectInventory, gameUse, gameSwitchBow, gameReset, gameShowScore, gameShowTable;
    private JMenuItem editorGoToLocation, editorGoToDestination, editorUpOneFloor, editorDownOneFloor, editorUpOneLevel,
	    editorDownOneLevel, editorAddLevel, editorRemoveLevel, editorResizeLevel, editorToggleLayer,
	    editorLevelPreferences, editorMazePreferences, editorSetStartPoint, editorSetFirstMovingFinish;
    private JMenuItem editorFillFloor, editorFillLevel, editorFillFloorRandomly, editorFillLevelRandomly,
	    editorFillRuleSets;
    ...
    private KeyStroke editUndoAccel, editRedoAccel, editCutLevelAccel, editCopyLevelAccel, editPasteLevelAccel,
	    editInsertLevelFromClipboardAccel, editPreferencesAccel, editClearHistoryAccel;
    private KeyStroke playPlayMazeAccel, playEditMazeAccel;
    private KeyStroke gameObjectInventoryAccel, gameUseAccel, gameSwitchBowAccel, gameResetAccel, gameShowScoreAccel,
	    gameShowTableAccel;
    private KeyStroke editorGoToLocationAccel, editorUpOneFloorAccel, editorDownOneFloorAccel, editorUpOneLevelAccel,
	...
	this.gameObjectInventoryAccel = KeyStroke.getKeyStroke(KeyEvent.VK_I, modKey);
	...
	this.editorUpOneFloorAccel = KeyStroke.getKeyStroke(KeyEvent.VK_UP, modKey);
	this.editorDownOneFloorAccel = KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, modKey);
	this.editorUpOneLevelAccel = KeyStroke.getKeyStroke(KeyEvent.VK_UP, modKey | InputEvent.SHIFT_DOWN_MASK);
	this.editorDownOneLevelAccel = KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, modKey | InputEvent.SHIFT_DOWN_MASK);
	this.editorToggleLayerAccel = KeyStroke.getKeyStroke(KeyEvent.VK_L, modKey);
    }
	...
	this.editorFillFloorRandomly = new JMenuItem("Fill Current Floor Randomly");
	this.editorFillLevelRandomly = new JMenuItem("Fill Current Level Randomly");
	...
	this.editorSetFirstMovingFinish = new JMenuItem("Set First Moving Finish...");
	this.pluginRegister = new JMenuItem("Register Plugin...");
	this.pluginUnregister = new JMenuItem("Unregister Plugin...");
	this.helpAbout = new JMenuItem("About WeaselWeb...");
	this.helpGeneralHelp = new JMenuItem("WeaselWeb Help");
	this.helpObjectHelp = new JMenuItem("WeaselWeb Object Help");
    ...
    public void dynAddMenuItems(final JMenuItem[] items, final String menuName, final boolean flag) {
    ...
    public void dynRemoveMenuItems(final JMenuItem[] items, final String menuName) {
	// Get Menu to remove items from
	JMenu menuToRemoveFrom;
	final boolean menuExists = this.doesMenuExist(menuName);
	if (menuExists) {
	    menuToRemoveFrom = this.getMenuWithName(menuName);
	    // Remove items from menu, along with action listener
	    for (final JMenuItem item : items) {
		this.removeMenuItemWithName(menuToRemoveFrom, item);
	    }
	    // Is menu now empty?
	    if (menuToRemoveFrom.getItemCount() == 0) {
		// Remove menu
		this.removeMenuWithName(menuToRemoveFrom);
	    }
	}
    }

    private void removeMenuWithName(final JMenu menu) {
	// Remove it from the menu bar
	this.mainMenuBar.remove(menu);
	// Rebuild the menu
	this.mainMenuBar.validate();
    }

    private void removeMenuItemWithName(final JMenu menu, final JMenuItem item) {
	// Remove listener
	item.removeActionListener(this.phandler);
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (cmd.equals("New...")) {
		    ...
		    app.getEditor().editTeleportDestination(MazeEditor.TELEPORT_TYPE_FIRST_MOVING_FINISH);
			...
			final int result = CommonDialogs
				.showConfirmDialog("Are you sure you want to reset the current level?", "WeaselWeb");
		...
		} else if (cmd.equals("About WeaselWeb...")) {
		    app.getAboutDialog().showAboutDialog();
		} else if (cmd.equals("WeaselWeb Object Help")) {

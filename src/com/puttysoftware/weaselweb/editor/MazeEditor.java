 ...
 */
package com.puttysoftware.weaselweb.editor;
...
import com.puttysoftware.weaselweb.maze.objects.ChainTeleport;
import com.puttysoftware.weaselweb.maze.objects.Empty;
import com.puttysoftware.weaselweb.maze.objects.EmptyVoid;
import com.puttysoftware.weaselweb.maze.objects.FinishTo;
import com.puttysoftware.weaselweb.maze.objects.InvisibleChainTeleport;
import com.puttysoftware.weaselweb.maze.objects.InvisibleOneShotChainTeleport;
import com.puttysoftware.weaselweb.maze.objects.InvisibleOneShotTeleport;
import com.puttysoftware.weaselweb.maze.objects.InvisibleTeleport;
import com.puttysoftware.weaselweb.maze.objects.MetalButton;
import com.puttysoftware.weaselweb.maze.objects.MovingFinish;
import com.puttysoftware.weaselweb.maze.objects.OneShotChainTeleport;
import com.puttysoftware.weaselweb.maze.objects.OneShotTeleport;
import com.puttysoftware.weaselweb.maze.objects.Player;
import com.puttysoftware.weaselweb.maze.objects.RandomInvisibleOneShotTeleport;
import com.puttysoftware.weaselweb.maze.objects.RandomInvisibleTeleport;
import com.puttysoftware.weaselweb.maze.objects.RandomOneShotTeleport;
import com.puttysoftware.weaselweb.maze.objects.RandomTeleport;
import com.puttysoftware.weaselweb.maze.objects.StairsDown;
import com.puttysoftware.weaselweb.maze.objects.StairsUp;
import com.puttysoftware.weaselweb.maze.objects.Teleport;
import com.puttysoftware.weaselweb.maze.objects.TreasureChest;
import com.puttysoftware.weaselweb.maze.objects.TwoWayTeleport;
...
public class MazeEditor {
    ...
    public static final int TELEPORT_TYPE_CONDITIONAL_CHAIN = 13;
    public static final int TELEPORT_TYPE_INVISIBLE_CONDITIONAL_CHAIN = 14;
	...
	if (w != 0) {
	    // Level Change
	    WeaselWeb.getApplication().getMazeManager().getMaze().switchLevelOffset(w);
	    this.fixLimits();
	    this.setUpGUI();
	}
	this.checkMenus();
	...
	this.elMgr.setEditorLocationW(w);
	// Level Change
	WeaselWeb.getApplication().getMazeManager().getMaze().switchLevel(w);
	this.fixLimits();
	this.setUpGUI();
	this.checkMenus();
	this.redrawEditor();
	...
	for (x = this.evMgr.getViewingWindowLocationX(); x <= this.evMgr.getLowerRightViewingWindowLocationX(); x++) {
		...
		try {
		    final BufferedImageIcon icon1 = app.getMazeManager().getMaze()
			    .getCell(y, x, this.elMgr.getEditorLocationZ(), MazeConstants.LAYER_GROUND)
			    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
		    this.drawGrid[xFix][yFix].setIcon(icon1);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    this.drawGrid[xFix][yFix].setIcon(ImageManager.getImage("Void"));
	...
	for (x = this.evMgr.getViewingWindowLocationX(); x <= this.evMgr.getLowerRightViewingWindowLocationX(); x++) {
		...
		try {
		    final BufferedImageIcon icon1 = app.getMazeManager().getMaze()
			    .getCell(y, x, this.elMgr.getEditorLocationZ(), MazeConstants.LAYER_GROUND)
			    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
		    final BufferedImageIcon icon2 = app.getMazeManager().getMaze()
    ...
    private void redrawVirtual(final int x, final int y, final BufferedImageIcon icon3) {
	// Draw the square
	final Application app = WeaselWeb.getApplication();
	int xFix, yFix;
	...
	final int gridX = x / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationX() - xOffset
		+ yOffset;
	final int gridY = y / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationY() + xOffset
		- yOffset;
	try {
	...
	try {
		...
		if (mo2 == null) {
		    WeaselWeb.getApplication().showMessage("This object has no properties");
		} else {
		    this.checkTwoWayTeleportPair(this.elMgr.getEditorLocationZ());
		    this.updateUndoHistory(this.savedMazeObject, gridX, gridY, this.elMgr.getEditorLocationZ(),
			    this.elMgr.getEditorLocationW(), this.elMgr.getEditorLocationE());
	...
	case TELEPORT_TYPE_INVISIBLE_ONESHOT_CHAIN:
	    WeaselWeb.getApplication().showMessage("Click to set teleport destination");
	    break;
	case TELEPORT_TYPE_MOVING_FINISH:
	    WeaselWeb.getApplication().showMessage("Click to set next moving finish");
	    break;
	case TELEPORT_TYPE_FIRST_MOVING_FINISH:
	    WeaselWeb.getApplication().showMessage("Click to set first moving finish");
	    break;
	...
	WeaselWeb.getApplication().showMessage("Click to set metal button target");
	final Application app = WeaselWeb.getApplication();
	this.horzScroll.removeAdjustmentListener(this.mhandler);
	this.vertScroll.removeAdjustmentListener(this.mhandler);
	this.secondaryPane.removeMouseListener(this.mhandler);
	...
	WeaselWeb.getApplication().showMessage("Pick treasure chest contents");
	...
	final int xOffset = this.vertScroll.getValue() - this.vertScroll.getMinimum();
	final int yOffset = this.horzScroll.getValue() - this.horzScroll.getMinimum();
	final int destX = x / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationX() - xOffset
		+ yOffset;
	final int destY = y / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationY() + xOffset
		- yOffset;
	...
	case TELEPORT_TYPE_GENERIC:
	    app.getMazeManager().getMaze().setCell(new Teleport(destX, destY, destZ), this.elMgr.getEditorLocationX(),
		    this.elMgr.getEditorLocationY(), this.elMgr.getCameFromZ(), MazeConstants.LAYER_OBJECT);
	    break;
	case TELEPORT_TYPE_INVISIBLE_GENERIC:
	...
	if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_MOVING_FINISH) {
	    WeaselWeb.getApplication().showMessage("Next moving finish set.");
	} else if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_FIRST_MOVING_FINISH) {
	    WeaselWeb.getApplication().showMessage("First moving finish set.");
	} else {
	    WeaselWeb.getApplication().showMessage("Destination set.");
	}
	app.getMazeManager().setDirty(true);
	this.redrawEditor();
    }
	...
	this.secondaryPane.addMouseListener(this.mhandler);
	this.checkMenus();
	WeaselWeb.getApplication().showMessage("Target set.");
	app.getMazeManager().setDirty(true);
	this.redrawEditor();
    }
	...
	app.getMazeManager().getMaze().setCell(new TreasureChest(contents), this.elMgr.getEditorLocationX(),
		this.elMgr.getEditorLocationY(), this.elMgr.getCameFromZ(), MazeConstants.LAYER_OBJECT);
	this.checkMenus();
	WeaselWeb.getApplication().showMessage("Contents set.");
	app.getMazeManager().setDirty(true);
	this.redrawEditor();
    }
	...
	WeaselWeb.getApplication().showMessage("Click to set start point");
    }
	...
	try {
	    ...
	    app.getMazeManager().getMaze().setCell(new Player(), destX, destY, this.elMgr.getEditorLocationZ(),
		    MazeConstants.LAYER_OBJECT);
	    WeaselWeb.getApplication().showMessage("Start point set.");
	} catch (final ArrayIndexOutOfBoundsException aioob) {
	...
	if (this.confirmNonUndoable("overwrite the active level with default data")) {
	    WeaselWeb.getApplication().getMazeManager().getMaze().fillLevelDefault();
	    WeaselWeb.getApplication().showMessage("Level filled.");
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with default data")) {
	    WeaselWeb.getApplication().getMazeManager().getMaze().fillFloorDefault(this.elMgr.getEditorLocationZ());
	    WeaselWeb.getApplication().showMessage("Floor filled.");
	    WeaselWeb.getApplication().getMazeManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable("overwrite the active level with random data")) {
	    if (WeaselWeb.getApplication().getMenuManager().useFillRuleSets()) {
		WeaselWeb.getApplication().getMazeManager().getMaze().fillLevelRandomlyCustom();
	    } else {
		WeaselWeb.getApplication().getMazeManager().getMaze().fillLevelRandomly();
	    }
	    WeaselWeb.getApplication().showMessage("Level randomly filled.");
	    WeaselWeb.getApplication().getMazeManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with random data")) {
	    ...
	    WeaselWeb.getApplication().showMessage("Floor randomly filled.");
	    WeaselWeb.getApplication().getMazeManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (!this.goToDestMode) {
	    ...
	    WeaselWeb.getApplication().showMessage("Click a teleport to go to its destination");
	}
    }
	...
	if (this.goToDestMode) {
	    ...
	    final int xOffset = this.vertScroll.getValue() - this.vertScroll.getMinimum();
	    final int yOffset = this.horzScroll.getValue() - this.horzScroll.getMinimum();
	    final int locX = x / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationX() - xOffset
		    + yOffset;
	    final int locY = y / ImageManager.getGraphicSize() + this.evMgr.getViewingWindowLocationY() + xOffset
		    - yOffset;
	    final int locZ = this.elMgr.getEditorLocationZ();
	    final MazeObject there = WeaselWeb.getApplication().getMazeManager().getMazeObject(locX, locY, locZ,
		    MazeConstants.LAYER_OBJECT);
		...
		this.redrawVirtual(destX, destY, ImageManager.getImage("destination"));
	    } else {
		WeaselWeb.getApplication().showMessage("This object does not have a destination.");
	    }
	}
    }
	...
	gm.stateChanged();
	WeaselWeb.getApplication().getGUIManager().showGUI();
    }
	...
	if (x != -1 && y != -1 && z != -1 && w != -1) {
	    ...
	    WeaselWeb.getApplication().showMessage("Nothing to undo");
	}
    }
	...
	if (x != -1 && y != -1 && z != -1 && w != -1) {
	    ...
	    WeaselWeb.getApplication().showMessage("Nothing to redo");
	}
    }

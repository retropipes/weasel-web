...
public class RotationTrap extends GenericTrap implements Cloneable {
	...
	WeaselWeb.getApplication().showMessage(this.getName() + " (Radius " + this.radius + ", Direction " + dir + ")");
    }
	...
	if (this.direction) {
	    WeaselWeb.getApplication().getGameManager().doClockwiseRotate(this.radius);
	} else {
	    WeaselWeb.getApplication().getGameManager().doCounterclockwiseRotate(this.radius);
	}

...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "WeaselWeb" + File.separator + randomID
		+ ".maze";
	...
	return System.getProperty("java.io.tmpdir") + File.separator + "WeaselWeb";
    }
	...
	if (this.activeLevel != level) {
		...
		try (final XDataWriter writer = this.getLevelWriter()) {
		    // Save old level
	    ...
	    try (final XDataReader reader = this.getLevelReader()) {
		// Load new level
	...
	try (final XDataReader metaReader = DataIOFactory.createTagReader(m.basePath + File.separator + "metafile.xml",
	    ...
	    // Create data reader
	    try (final XDataReader dataReader = m.getLevelReader()) {
		// Read data
    ...
    private void readMazeLevel(final XDataReader reader, final int formatVersion) throws IOException {
	if (formatVersion == FormatConstants.MAZE_FORMAT_1) {
	    this.mazeData = LayeredTower.readLayeredTower(reader, formatVersion);
	...
	// Create data writer
	try (final XDataWriter dataWriter = this.getLevelWriter()) {
	    // Write data

...
public class MazeEffectManager {
	...
	this.activeEffectMessageContainer.setLayout(new GridLayout(MazeEffectManager.MAX_ACTIVE_EFFECTS, 1));
	...
	for (int x = 0; x < MazeEffectManager.NUM_EFFECTS; x++) {
		...
		if (!this.activeEffects[x].isActive()) {
		    WeaselWeb.getApplication().showMessage("You feel normal again.");
		    // Clear effect grid
		    this.clearGridEntry(x);
		    // Pack
		    WeaselWeb.getApplication().getGameManager().getOutputFrame().pack();
		}
	    }
	}
    }

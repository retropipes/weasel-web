...
import com.puttysoftware.weaselweb.maze.objects.BarrierGenerator;
import com.puttysoftware.weaselweb.maze.objects.CrackedWall;
import com.puttysoftware.weaselweb.maze.objects.Crevasse;
...
import com.puttysoftware.weaselweb.maze.objects.EnragedBarrierGenerator;
import com.puttysoftware.weaselweb.maze.objects.ForceField;
import com.puttysoftware.weaselweb.maze.objects.HotRock;
import com.puttysoftware.weaselweb.maze.objects.Ice;
import com.puttysoftware.weaselweb.maze.objects.IcedBarrierGenerator;
...
import com.puttysoftware.weaselweb.maze.objects.PoisonedBarrierGenerator;
import com.puttysoftware.weaselweb.maze.objects.ShockedBarrierGenerator;
import com.puttysoftware.weaselweb.maze.objects.Slime;
...
class LayeredTower implements Cloneable {
	...
	if (this.timerActive) {
		...
		WeaselWeb.getApplication().getGameManager().solvedLevel();
	    }
	}
    }
	...
	boolean result = true;
	final int zLoc = WeaselWeb.getApplication().getGameManager().getPlayerManager().getPlayerLocationZ();
    ...
    public static LayeredTower readLayeredTower(final XDataReader reader, final int ver) throws IOException {
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(WeaselWeb.getApplication().getObjects().readMazeObject(reader, ver), y, x, z, e);
	...
	this.savedTowerState = SavedTowerState.readSavedTowerState(reader, formatVersion);
    }
}

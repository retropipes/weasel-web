...
import com.puttysoftware.weaselweb.maze.objects.HorizontalBarrier;
import com.puttysoftware.weaselweb.maze.objects.VerticalBarrier;
...
public abstract class GenericGenerator extends GenericWall {
	...
	final MazeObject mo2 = app.getMazeManager().getMazeObject(dirX - 1, dirY, pz, MazeConstants.LAYER_OBJECT);
	try {
	    mo2Name = mo2.getName();
	...
	final MazeObject mo4 = app.getMazeManager().getMazeObject(dirX, dirY - 1, pz, MazeConstants.LAYER_OBJECT);
	try {
	    mo4Name = mo4.getName();
	} catch (final NullPointerException np) {
	    mo4Name = invalidName;
	...
	final MazeObject mo6 = app.getMazeManager().getMazeObject(dirX, dirY + 1, pz, MazeConstants.LAYER_OBJECT);
	try {
	    mo6Name = mo6.getName();
	} catch (final NullPointerException np) {
	    mo6Name = invalidName;
	...
	final MazeObject mo8 = app.getMazeManager().getMazeObject(dirX + 1, dirY, pz, MazeConstants.LAYER_OBJECT);
	try {
	    mo8Name = mo8.getName();
	} catch (final NullPointerException np) {
	    mo8Name = invalidName;
	...
	if (flag) {
	    SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE, SoundConstants.SOUND_GENERATE);
	    this.activateTimer(this.TIMER_DELAY);
	...
	if (dir == DirectionConstants.DIRECTION_EAST) {
		...
		final MazeObject mo = app.getMazeManager().getMazeObject(x + l, y, z, MazeConstants.LAYER_OBJECT);
		String moName;
		try {
		    moName = mo.getName();
		} catch (final NullPointerException np) {
	    ...
	    for (int l = 1; l < limit; l++) {
		final MazeObject mo = app.getMazeManager().getMazeObject(x, y - l, z, MazeConstants.LAYER_OBJECT);
		String moName;
		try {
		    moName = mo.getName();
		} catch (final NullPointerException np) {
	...
	if (dir == DirectionConstants.DIRECTION_EAST) {
		...
		if (o) {
			...
			try {
			    app.getMazeManager().getMaze().setCell(new HorizontalBarrier(), x + l, y, z,
			...
			try {
			    app.getMazeManager().getMaze().setCell(new Empty(), x + l, y, z,
				    MazeConstants.LAYER_OBJECT);
			} catch (final ArrayIndexOutOfBoundsException aioob) {
			    // Do nothing
			}
		...
		if (o) {
			...
			try {
			    app.getMazeManager().getMaze().setCell(new VerticalBarrier(), x, y - l, z,
				    MazeConstants.LAYER_OBJECT);
			} catch (final ArrayIndexOutOfBoundsException aioob) {
			    // Do nothing
			}
			...
			try {
			    app.getMazeManager().getMaze().setCell(new Empty(), x, y - l, z,
				    MazeConstants.LAYER_OBJECT);
			} catch (final ArrayIndexOutOfBoundsException aioob) {
			    // Do nothing
			}

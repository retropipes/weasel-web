...
import com.puttysoftware.weaselweb.maze.FormatConstants;
import com.puttysoftware.weaselweb.maze.MazeConstants;
import com.puttysoftware.weaselweb.maze.objects.*;
...
public class MazeObjectList {
    ...
    private final MazeObject[] allObjects = { new Empty(), new Grass(), new Dirt(), new Sand(), new Snow(),
	    ...
	    new Tablet(), new TabletSlot(), new EnergySphere(), new GarnetSquare(), new GarnetWall(),
	    new GoldenSquare(), new GoldenWall(), new RubySquare(), new RubyWall(), new SapphireSquare(),
	    new SapphireWall(), new SilverSquare(), new SilverWall(), new TopazSquare(), new TopazWall(), new Key(),
	    new Lock(), new BlueKey(), new BlueLock(), new GreenKey(), new GreenLock(), new MagentaKey(),
	    new MagentaLock(), new OrangeKey(), new OrangeLock(), new PurpleKey(), new PurpleLock(), new RedKey(),
	    new RedLock(), new RoseKey(), new RoseLock(), new SeaweedKey(), new SeaweedLock(), new SkyKey(),
	    new SkyLock(), new WhiteKey(), new WhiteLock(), new YellowKey(), new YellowLock(), new CyanKey(),
	    new CyanLock(), new MetalKey(), new MetalDoor(), new Door(), new BlueButton(), new GreenButton(),
	    new MagentaButton(), new OrangeButton(), new PurpleButton(), new RedButton(), new RoseButton(),
	    new SeaweedButton(), new SkyButton(), new WhiteButton(), new YellowButton(), new CyanButton(),
	    new MetalButton(), new Teleport(), new InvisibleTeleport(), new RandomTeleport(),
	    new RandomInvisibleTeleport(), new RandomOneShotTeleport(), new RandomInvisibleOneShotTeleport(),
	    new OneShotTeleport(), new InvisibleOneShotTeleport(), new TwoWayTeleport(), new ControllableTeleport(),
	    new OneShotControllableTeleport(), new ConditionalTeleport(), new InvisibleConditionalTeleport(),
	    new OneShotConditionalTeleport(), new InvisibleOneShotConditionalTeleport(), new ChainTeleport(),
	    new InvisibleChainTeleport(), new OneShotChainTeleport(), new InvisibleOneShotChainTeleport(),
	    new ConditionalChainTeleport(), new InvisibleConditionalChainTeleport(), new StairsUp(), new StairsDown(),
	    new Pit(), new InvisiblePit(), new Springboard(), new InvisibleSpringboard(), new PushableBlock(),
	    new PullableBlock(), new PushablePullableBlock(), new PushableBlockOnce(), new PushableBlockTwice(),
	    new PushableBlockThrice(), new PullableBlockOnce(), new PullableBlockTwice(), new PullableBlockThrice(),
	    new MovingBlock(), new MetalBoots(), new NoBoots(), new HealBoots(), new GlueBoots(), new AquaBoots(),
	    new BioHazardBoots(), new FireBoots(), new HotBoots(), new PasswallBoots(), new SlipperyBoots(),
	    new AnnihilationWand(), new FinishMakingWand(), new WallMakingWand(), new TeleportWand(),
	    new WallBreakingWand(), new DisarmTrapWand(), new RemoteActionWand(), new RotationWand(), new WarpWand(),
	    new LightWand(), new DarkWand(), new EmptyVoid(), new HealTrap(), new HurtTrap(), new VariableHealTrap(),
	    ...
	    new SeaweedHouse(), new SkyHouse(), new WhiteHouse(), new YellowHouse(), new Exit() };
	...
	for (int x = 0; x < allEditorAppearances.length; x++) {
	    allEditorAppearances[x] = ImageManager.getTransformedImage(this.allObjects[x].getName());
	}
	...
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].getLayer() == MazeConstants.LAYER_GROUND) {
		tempAllGroundLayerEditorAppearances[x] = ImageManager.getTransformedImage(this.allObjects[x].getName());
	    }
	}
	...
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].getLayer() == MazeConstants.LAYER_OBJECT) {
		tempAllObjectLayerEditorAppearances[x] = ImageManager.getTransformedImage(this.allObjects[x].getName());
	    }
	}
	...
	for (int x = 0; x < this.allObjects.length; x++) {
		...
		tempAllContainableObjectEditorAppearances[x] = ImageManager
	...
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].isInventoryable()) {
		tempAllInventoryableNames[x] = this.allObjects[x].getPluralName();
	    }
	}
	for (final String tempAllInventoryableName : tempAllInventoryableNames) {
	    if (tempAllInventoryableName != null) {
    ...
    public String[] getAllInventoryablePluralNamesMinusSpecial() {
	final String[] tempAllInventoryableNames = new String[this.allObjects.length];
	int objectCount = 0;
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].isInventoryable() && !this.allObjects[x].isOfType(TypeConstants.TYPE_BOOTS)
    ...
    public MazeObject[] getAllUsableObjectsMinusSpecial() {
	final MazeObject[] tempAllUsableObjects = new MazeObject[this.allObjects.length];
	int objectCount = 0;
	for (int x = 0; x < this.allObjects.length; x++) {
    ...
    public String[] getAllUsablePluralNamesMinusSpecial() {
	final String[] tempAllUsableNames = new String[this.allObjects.length];
	int objectCount = 0;
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].isUsable() && !this.allObjects[x].isOfType(TypeConstants.TYPE_BOW)) {
    ...
    public String[] getAllBowPluralNames() {
	final String[] tempAllUsableNames = new String[this.allObjects.length];
	int objectCount = 0;
	for (int x = 0; x < this.allObjects.length; x++) {
	    if (this.allObjects[x].isOfType(TypeConstants.TYPE_BOW)) {
	...
	allUsableNames[allUsableNames.length - 1] = new Bow().getPluralName();
	return allUsableNames;
    }
    ...
    public MazeObject readMazeObject(final XDataReader reader, final int formatVersion) throws IOException {
	MazeObject o = null;
	String UID = "";
	...
	for (int x = 0; x < mapLen; x++) {
		...
		this.allObjects[x].getRuleSet().readRuleSet(reader, rsFormat);
	    }
	}
    }
